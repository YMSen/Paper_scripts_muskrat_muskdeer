...
sub parse
{
	...
	s/(\(|((\)?[^,;:\'\[\]\(\)]+|\))(\'[\>\<\=\d.eE\-\s\*]+\'\s*)?(:[\d.eE\-]+)?(\[&&NHX[^\[\]]*\])?))/&parse_aux($self,$array,\@stack,$1,$3,$4,$5,$6)/eg;
	if (@stack != 1) {
		my $count = @stack;
...
sub parse_aux
{
	my ($self, $array, $stack, $str, $name, $interval, $dist, $nhx) = @_;
	if ($str eq '(') {
		push(@$stack, $str);
	} elsif ($name) {
		my %hash;
		...
		if ($interval) {
			$interval =~ s/^[\s\']+|[\s\']+$//g;
			$interval =~ s/\s+//g;
			my @arr1 = split /-/, $interval;
			$interval = sprintf "%.1f-%.1f", $arr1[0], $arr1[1];
