...
{	
	...
	my $self = Tree::nhx->new(
		...
		font=>'Times-Bold',			fsize=>16,
		ruler_fsize=>12,			node_fsize=>10,
		c_bg=>"#FFFFFF",			c_frame=>"#FF00CC", 
		c_inter=>"#2255AA",			c_exter=>"#000000",
...
sub cal_xy{
	...
	if ($self->{xskip} > 0){
		$self->{width} = $self->{height} * $self->{xskip} / $self->{skip} 
			+ $max + $self->{left_margin} + $self->{right_margin};
	}
...
sub plot_ruler{
	...
	if ($pos eq '' || $pos eq "left") {
		$svg->text('x',$X_start-textWidth($self->{font},$self->{ruler_fsize},$type)-4,
			'y',$Y,'-cdata',$type,"font-family",$self->{font},"font-size",$self->{ruler_fsize},"fill",'#000000');
	}
	if ($pos eq "right") {
		$svg->text('x',$X_end + 4,'y',$Y,'-cdata',$type,"font-family",$self->{font},"font-size",$self->{ruler_fsize},"fill",'#000000');
	}
	...
	for (my $i=0; $i<=$len; $i+=$unit/4) { #5
		
		my $X = $X_end - ($i / $len * ($X_end - $X_start));
	...
	for (my $i=0; $i<=$len; $i+=$unit) {
		...
		$svg->text('x',$X - textWidth($self->{font},$self->{ruler_fsize},$i) / 2,'y',$Y+textHeight($self->{ruler_fsize})+4,'fill','#000000','-cdata',$i,'font-size',$self->{ruler_fsize}, 'font-family',$self->{font});
	}
...
sub cal_cumu_dist {
	...
	foreach my $p (@$array) {
		...
		elsif (exists $p->{C}->[0]->{cumu_dist}) {
			$p->{cumu_dist} = $p->{C}->[0]->{cumu_dist} + $p->{C}->[0]->{dist};
		}
	}
	$self->{_root}->{cumu_dist} = $self->{_root}->{C}->[0]->{cumu_dist} + $self->{_root}->{C}->[0]->{dist};
}
...
sub plot_core{
	...
	foreach my $p (@$array) {
		...
		&& $p->{N}) {
			...
			my ($outname, $font_style) = ($p->{N}, "normal");
			if (defined $p->{common}) {
				$outname = $p->{common};
			}
			elsif (defined $p->{scientific}) {
				$outname = $p->{scientific};
				$font_style = "italic";
			}

			$svg->text('x',$p->{x}+$self->{half_box}+4,'y',$p->{y}+textHeight($self->{fsize})/2,'fill',$self->{c_exter},'-cdata',$outname,'font-size',$self->{fsize}, 'font-family',$self->{font}, 'font-style', $font_style);
		}
	}
	...
	# draw interval string for internal nodes
	$self->cal_cumu_dist;
	foreach my $p (@$array) {
		if ($self->{show_interval} && $p->{C} && ($p->{dist} || $p->{interval})){
			my $tmpstr = sprintf "%.1f", $p->{cumu_dist};
			$tmpstr .= "($p->{interval})";
			$tmpstr =~ s/[\s\*]+//g;
			$tmpstr =~ s/\(\s*\)//g;
			my $deco = "none";
			#$deco = "underline" if ($p->{interval} =~ /\*$/);
			$svg->text('x',$p->{x}+3,
				'y',$p->{y}+4,'fill',$self->{c_inter},'-cdata',$tmpstr,
				'font-size',$self->{node_fsize}, 'font-family',$self->{font},
				'text-decoration',$deco,
			); 
			if ($p->{interval} =~ /\*$/) {
				#my $color = (exists $p->{D} && $p->{D} eq "Y") ? "red" : $self->{c_node};
				my $color = $self->{c_node};
				$svg->circle('cx',$p->{x}, 'cy',$p->{y},'r',$self->{half_box},'fill',$color);
				#$svg->rect('x',$p->{x}-$self->{half_box}, 'y',$p->{y}-$self->{half_box},
				#	'width',$self->{half_box}*2,'height',$self->{half_box}*2,'fill',$color);
			}
			#print STDERR "$tmpstr\n";
		}	
	}
	...
	#$self->plot_ruler( $ruler_y,$self->{left_margin}, $self->{left_margin} + $self->{tree_width}, 
	#$self->{tree_dist}, "dist" ) if($self->{show_ruler} && $self->{tree_width} );
	$self->plot_ruler( $ruler_y,$self->{left_margin}, $self->{left_margin} + $self->{tree_width}, 
		$self->{tree_dist}, "Million years ago", "right" ) if($self->{show_ruler} && $self->{tree_width} );
	...
	foreach my $p (@$array) {
		if($self->{show_W} && defined $p->{W}) {
			...
			my $detail = "Dn=$p->{Dn};\\nDs=$p->{Ds};\\nDn/Ds=$p->{W}";
			my $sw = textWidth($self->{font},$self->{fsize},$W_str);
			my $x = ($p->{x} - $self->{half_box} - $p->{P}->{x}  >= $sw) ? ($p->{P}->{x} + $p->{x} - $self->{half_box} - $sw)/2 : ($p->{x} - $self->{half_box} - $sw ) ;
			$svg->text('x',$x,'y',$p->{y}-4,'fill',$color,'-cdata',$W_str,'font-size',$self->{fsize}, 'font-family',$self->{font}, 'onclick',"alert('$detail')");
			@{$p->{kaks_area}} = ( int($x), int($p->{y}-4 - textHeight($self->{fsize})), 
				int($x+textWidth($self->{font},$self->{fsize},$W_str)) , int($p->{y}-4) );	
		}
	}
...
sub add_cali {
	...
	my $nhx_tmp = shift;
	my $arr1 = $self->{_node};
	my @arr2 = $nhx_tmp->node();
	foreach my $p (@$arr1)
	{
		my $p2 = shift @arr2;
		if ($p->{C} && $p2->{C})
		{
			$p->{interval} .= "*" if (exists $p2->{interval} && $p2->{interval} ne "");
		}
	}
	return $self;
}
