...
use lib "/PUBLIC/software/DENOVO/bio/annotation/pipeline_v2.0/perllib";
use pply;
...
my $blat_path = "/ifs/TJPROJ3/DENOVO/leiyang/study/pipeline_v3.0/scripts/blat.pl";
my $blat_orig = "/PUBLIC/software/public/Alignment/blat/blat";
...
my $ooc_cmd = "$blat_orig $genome_db $transcript_db -q=rna -dots=100 -maxIntron=$MAX_INTRON  -makeOoc=11.ooc /dev/null";

$genome_db = abspath $genome_db;
if($genome_db =~ m|(.*)/(.*?)$|){
	my ($dir,$file) = ($1,$2);
	unless(-s "./_$file.cut.list"){
		&process_cmd("/ifs/TJPROJ3/DENOVO/leiyang/study/pipeline_v3.0/scripts/fastaDeal.pl -cutf 20 $genome_db -outdir ./");
	}
	$genome_db = abspath $file;
}
...

{
    ...
    foreach my $transcript_file (@transcript_files) {
        ...
        $transcript_file = abspath $transcript_file;
        ## process blat search:
        my $cmd = "$blat_path $genome_db $transcript_file -q=rna -dots=100 "
            . " -maxIntron=$MAX_INTRON -out=pslx -ooc=$ENV{'PWD'}/11.ooc $transcript_file.pslx";
