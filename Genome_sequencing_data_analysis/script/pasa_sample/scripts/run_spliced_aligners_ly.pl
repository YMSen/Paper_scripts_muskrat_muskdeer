...
#                            Currently supported: gmap, blat, blat_ly
#
...
my %SUPPORTED_ALIGNERS = map { + $_ => 1 } qw(blat gmap blat_ly);
...
main: {
    ...
    if ($aligners =~ /blat_ly/i) {
        $thread_helper->wait_for_open_thread();
        my $thread = threads->create('run_blat_ly');
        $thread_helper->add_thread($thread);
    }elsif ($aligners =~ /blat/i) {
        $thread_helper->wait_for_open_thread();
...
sub run_blat_ly {
    ...
    my $cmd = "$FindBin::Bin/process_BLAT_ly_alignments.pl -g $genome_db "
        . " -t $transcripts_db -I $max_intron_length -o blat_ly.spliced_alignments -N $num_top_hits --CPU $CPU ";
