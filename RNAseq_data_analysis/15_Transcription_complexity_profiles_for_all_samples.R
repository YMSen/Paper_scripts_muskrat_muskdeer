...
for (order in c(1:nrow(info))) {
  ...
  data_one_sample = as.data.frame(matrix(ncol = 5, nrow = length(data_1)))
  colnames(data_one_sample) = c("Order","Proportion_cumulative_expression","Sample","Tissue","Expression")
  data_one_sample[["Expression"]] = data_1
  data_one_sample[["Order"]] = c(1:nrow(data_one_sample))
  data_one_sample[["Sample"]] = info[order,"sample"]
  data_one_sample[["Tissue"]] = info[order,"condition"]
  for (number in c(1:nrow(data_one_sample))) {
    if (number == 1){
      data_one_sample[["Proportion_cumulative_expression"]][number] = data_one_sample[["Expression"]][number]/sum(data_one_sample[["Expression"]])*100
    }
    if (number != 1){
      data_one_sample[["Proportion_cumulative_expression"]][number] = data_one_sample[["Expression"]][number]/sum(data_one_sample[["Expression"]])*100 + data_one_sample[["Proportion_cumulative_expression"]][number-1]
    }
  }
  data = rbind(data, data_one_sample)
  data = na.omit(data)
}

write.table(data, file = "Summary_of_cumulative_percentage_of_gene_expression_for_all_samples.txt", col.names = T, row.names = F, sep = "\t", quote = F)


data_plot = read.table(file = "Summary_of_cumulative_percentage_of_gene_expression_for_all_samples.txt", header = T, check.names = F)
P = ggplot(data_plot, aes(Order, Proportion_cumulative_expression))+
  geom_line(aes(colour = Tissue, linetype = Tissue, group = Sample), linewidth = 0.25)+
...
ggsave(P, filename = "Transcription complexity analysis of all samples.pdf", width = 6, height = 3.5)
