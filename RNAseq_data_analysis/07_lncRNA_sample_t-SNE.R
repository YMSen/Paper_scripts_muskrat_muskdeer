...
data = log2(data+1)
genes = names(head(sort(apply(data,1,function(x) 100*sd(x)/mean(x)),decreasing=T),25000))

data = as.data.frame(t(data[genes,]))
data$class = info$condition
data = as.data.frame(data)
data_unique = unique(data)
data_matrix = as.matrix(data_unique[,1:25000])

set.seed(125)
tsne_out = Rtsne(data_matrix, pca=T, dims=2, perplexity=22, theta=0.65)
...
ggsave(P, filename = "lncRNA_t-SNE_plot.pdf", width = 6.8, height = 6)
