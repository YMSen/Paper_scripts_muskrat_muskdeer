...
data = read.table(file = "./00_files/Summary_of_the_lengths_of_the_three_versions_muskrat.txt", header = T, check.names = F)

data_plot = as.data.frame(matrix(ncol = 3, nrow = 1))
colnames(data_plot) = c("Length","Version","Position_percentage")

for (version in c("First","Second","Third")) {
...
P = ggplot(data_plot, aes(Position_percentage, Length/1000000))+ 
  ...
  scale_color_manual(values=c(First = "#56a0d3", Second = "#8ec06c", Third = '#ecb731'))+
  theme_test()+
