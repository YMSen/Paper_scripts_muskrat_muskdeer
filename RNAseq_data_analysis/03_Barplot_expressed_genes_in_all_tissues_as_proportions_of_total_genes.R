...
data_exp = read.table(file = "./00_files/All_samples_tpm.txt", header = T, check.names = F, row.names = 1)
...
colnames(data) = c("Sample","Tissue","Proportion_PCG")
...
for (order in c(1:nrow(info))) {
  ...
  data[["Proportion_PCG"]][order] = length(data_1)/nrow(data_exp)*100
}


data_plot_1 = data
mean <- aggregate(data_plot_1[["Proportion_PCG"]], by=list(data_plot_1[["Tissue"]]), FUN=mean)
colnames(mean) = c("Tissue","mean")
sd <- aggregate(data_plot_1[["Proportion_PCG"]], by=list(data_plot_1[["Tissue"]]), FUN=sd)
...
P = ggplot()+
  ...
  geom_jitter(data = data_plot_1, aes(x=Tissue, y=Proportion_PCG, fill=Tissue), size = 1.2, shape = 21,
...
ggsave(P, filename = "Proportion of expression gene expression in all tissue.pdf", width = 6, height = 3.5)
